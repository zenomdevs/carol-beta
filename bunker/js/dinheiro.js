const fs = require("fs")
const sabrpg = JSON.parse(fs.readFileSync("./bunker/database/usuarios/SystemRPG/sabrpg.json"));

exports.AdicionarGold = (UserID, ValorA) => {
userNumber = sabrpg.map(i => i.id).indexOf(UserID)
sabrpg[userNumber].money += Number(ValorA)
fs.writeFileSync("./bunker/database/usuarios/SystemRPG/sabrpg.json", JSON.stringify(sabrpg))
}
