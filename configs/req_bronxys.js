...
apiUrl.searchParams.append("AnnaV8", this.#key);
...
threads(url) {
return `${this.#base}/threads?apikey=${this.#key}&url=${url}`;
}
...
sfundo(url) {
return `${this.#base}/sfundo?url=${url}&apikey=${this.#key}`;
}

gerarlink_xatimg() {
return `${this.#base}/link_imagem?apikey=${this.#key}`;
}
...
soundcloud(url) {
return `${this.#base}/soundcloud?url=${url}&apikey=${this.#key}`;
}
